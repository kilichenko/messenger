...
class ResponseCreator:
    ...
    # expects content {key: str, public_key: str}
    def _connect(self):
        try:
            message_recipient = Users.get_user(self.content['recipient'])
            if not message_recipient.has_pending_connect_from(self.content['sender']):
                message = UserMessage(recipient=self.content['recipient'],
                                      sender=self.content['sender'],
                                      content=self.content['content'])
                message.set_timestamp()
                ...
                self.status = ResponseCreator.status_codes['CONNECT_ALREADY_PENDING']
        except KeyError:
            self.status = ResponseCreator.status_codes['USER_DO_NOT_EXIST']
    ...
    def process_request(self) -> Dict:
        ...
        if request_is_valid:
            ...
            print('decoded request: ', type(self.content), self.content)
